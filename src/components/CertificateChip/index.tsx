...
import { Base64String, CertificateDefinitionData, CertificateFieldDescriptor } from '@bsv/sdk'
import { WalletContext } from '../../WalletContext'
import { deterministicImage } from '../../utils/deterministicImage'
import AppLogo from '../AppLogo'
interface CertificateChipProps {
...
function isoFromEpochSeconds(secs?: number): string | undefined {
  // 0 means never expires - don't show any date
...
function relativeFromEpochSeconds(secs?: number): string | undefined {
  // 0 means never expires - don't show any text
  if (secs === 0) return undefined
  if (!secs && secs !== 0) return undefined
...
const CertificateChip: React.FC<CertificateChipProps> = ({
  ...
  const { settings, activeProfile, clients } = useContext(WalletContext)
  const registrant = clients.registryFromWallet
  ...
  useEffect(() => {
    ...
    const registryOperators: string[] = trusted.map((x: any) => x.identityKey)
      ...
      try {
        ...
        if (cachedData) {
          try {
            ...
            console.warn('Failed to parse cached certificate definition', error)
  ...
  }, [registrant, settings?.trustSettings?.trustedCertifiers, certType, activeProfile])
  ...
  const onVerifierClick = useCallback((e?: React.MouseEvent) => {
    e?.stopPropagation()
    navigate(`/dashboard/counterparty/${encodeURIComponent(certVerifier)}`)
  }, [navigate, certVerifier])
  /* ---------- click navigation ---------- */
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    navigate(`/dashboard/certificate/${encodeURIComponent(typeForRoute)}`)
  }
  if(isLoading)
  {
...
export default CertificateChip
