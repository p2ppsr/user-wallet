interface DashboardStyleOptions {
  breakpoints?: { sm?: boolean; xs?: boolean }
}

export default (theme: any, options: DashboardStyleOptions = {}) => ({
  content_wrap: {
    ...
    '& > :last-child': {
      ...
      '@media (min-width: 1500px)': {
        margin: (options.breakpoints?.sm || options.breakpoints?.xs) ? '0' : 'auto'
      }
    }
  },
  list_wrap: {
